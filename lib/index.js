    ...
    return new Promise( (resolve,reject) => {
        ...
        if (opts.by_id) {
            ...
            client.userStreaming()
                .then(stream => {
                    ...
                    stream.on('notification', (notification) => {
                        ...
                        if (! type) return;
                        // allowlist (opts.include) wins; else fall back to the denylist
                        const keep = (opts.include && opts.include.length)
                            ? opts.include.includes(type)
                            : ! opts.exclude.includes(type);
                        if (keep) {
            ...
            client.getNotifications({
                ...
                .then((res) => {
                    logger.debug(res);
                    // megalodon's getNotifications only forwards exclude_types (denylist);
                    // apply the allowlist client-side so MASTODON_INCLUDE_TYPES works here too.
                    const data = (opts.include && opts.include.length)
                        ? res.data.filter( (n) => opts.include.includes(n.type) )
                        : res.data;
                    resolve(data);
                })
                .catch(err => reject(err));
